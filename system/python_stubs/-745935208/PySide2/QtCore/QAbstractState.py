# encoding: utf-8
# module PySide2.QtCore
# from C:\Users\siddh\AppData\Local\Programs\Python\Python37\lib\site-packages\PySide2\QtCore.pyd
# by generator 1.146
# no doc

# imports
import Shiboken as __Shiboken


from .QObject import QObject

class QAbstractState(QObject):
    # no doc
    def active(self, *args, **kwargs): # real signature unknown
        pass

    def activeChanged(self, *args, **kwargs): # real signature unknown
        pass

    def entered(self, *args, **kwargs): # real signature unknown
        pass

    def event(self, *args, **kwargs): # real signature unknown
        pass

    def exited(self, *args, **kwargs): # real signature unknown
        pass

    def machine(self, *args, **kwargs): # real signature unknown
        pass

    def onEntry(self, *args, **kwargs): # real signature unknown
        pass

    def onExit(self, *args, **kwargs): # real signature unknown
        pass

    def parentState(self, *args, **kwargs): # real signature unknown
        pass

    def __init__(self, *args, **kwargs): # real signature unknown
        pass

    @staticmethod # known case of __new__
    def __new__(*args, **kwargs): # real signature unknown
        """ Create and return a new object.  See help(type) for accurate signature. """
        pass

    staticMetaObject = None # (!) real value is ''


