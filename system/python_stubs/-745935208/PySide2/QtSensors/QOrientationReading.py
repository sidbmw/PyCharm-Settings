# encoding: utf-8
# module PySide2.QtSensors
# from C:\Users\siddh\AppData\Local\Programs\Python\Python37\lib\site-packages\PySide2\QtSensors.pyd
# by generator 1.146
# no doc

# imports
import PySide2.QtCore as __PySide2_QtCore
import Shiboken as __Shiboken


from .QSensorReading import QSensorReading

class QOrientationReading(QSensorReading):
    # no doc
    def copyValuesFrom(self, *args, **kwargs): # real signature unknown
        pass

    def orientation(self, *args, **kwargs): # real signature unknown
        pass

    def setOrientation(self, *args, **kwargs): # real signature unknown
        pass

    def __init__(self, *args, **kwargs): # real signature unknown
        pass

    @staticmethod # known case of __new__
    def __new__(*args, **kwargs): # real signature unknown
        """ Create and return a new object.  See help(type) for accurate signature. """
        pass

    FaceDown = None # (!) real value is ''
    FaceUp = None # (!) real value is ''
    LeftUp = None # (!) real value is ''
    Orientation = None # (!) real value is ''
    RightUp = None # (!) real value is ''
    staticMetaObject = None # (!) real value is ''
    TopDown = None # (!) real value is ''
    TopUp = None # (!) real value is ''
    Undefined = None # (!) real value is ''


