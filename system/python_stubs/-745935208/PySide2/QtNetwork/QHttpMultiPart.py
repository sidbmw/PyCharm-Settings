# encoding: utf-8
# module PySide2.QtNetwork
# from C:\Users\siddh\AppData\Local\Programs\Python\Python37\lib\site-packages\PySide2\QtNetwork.pyd
# by generator 1.146
# no doc

# imports
import PySide2.QtCore as __PySide2_QtCore
import Shiboken as __Shiboken


class QHttpMultiPart(__PySide2_QtCore.QObject):
    # no doc
    def append(self, *args, **kwargs): # real signature unknown
        pass

    def boundary(self, *args, **kwargs): # real signature unknown
        pass

    def setBoundary(self, *args, **kwargs): # real signature unknown
        pass

    def setContentType(self, *args, **kwargs): # real signature unknown
        pass

    def __init__(self, *args, **kwargs): # real signature unknown
        pass

    @staticmethod # known case of __new__
    def __new__(*args, **kwargs): # real signature unknown
        """ Create and return a new object.  See help(type) for accurate signature. """
        pass

    AlternativeType = None # (!) real value is ''
    ContentType = None # (!) real value is ''
    FormDataType = None # (!) real value is ''
    MixedType = None # (!) real value is ''
    RelatedType = None # (!) real value is ''
    staticMetaObject = None # (!) real value is ''


