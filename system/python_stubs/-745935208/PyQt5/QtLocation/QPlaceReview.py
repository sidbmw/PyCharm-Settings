# encoding: utf-8
# module PyQt5.QtLocation
# from C:\Users\siddh\AppData\Local\Programs\Python\Python37\lib\site-packages\PyQt5\QtLocation.pyd
# by generator 1.146
# no doc

# imports
import PyQt5.QtCore as __PyQt5_QtCore
import sip as __sip


from .QPlaceContent import QPlaceContent

class QPlaceReview(QPlaceContent):
    """
    QPlaceReview()
    QPlaceReview(QPlaceContent)
    QPlaceReview(QPlaceReview)
    """
    def dateTime(self): # real signature unknown; restored from __doc__
        """ dateTime(self) -> QDateTime """
        pass

    def language(self): # real signature unknown; restored from __doc__
        """ language(self) -> str """
        return ""

    def rating(self): # real signature unknown; restored from __doc__
        """ rating(self) -> float """
        return 0.0

    def reviewId(self): # real signature unknown; restored from __doc__
        """ reviewId(self) -> str """
        return ""

    def setDateTime(self, Union, QDateTime=None, datetime_datetime=None): # real signature unknown; restored from __doc__
        """ setDateTime(self, Union[QDateTime, datetime.datetime]) """
        pass

    def setLanguage(self, p_str): # real signature unknown; restored from __doc__
        """ setLanguage(self, str) """
        pass

    def setRating(self, p_float): # real signature unknown; restored from __doc__
        """ setRating(self, float) """
        pass

    def setReviewId(self, p_str): # real signature unknown; restored from __doc__
        """ setReviewId(self, str) """
        pass

    def setText(self, p_str): # real signature unknown; restored from __doc__
        """ setText(self, str) """
        pass

    def setTitle(self, p_str): # real signature unknown; restored from __doc__
        """ setTitle(self, str) """
        pass

    def text(self): # real signature unknown; restored from __doc__
        """ text(self) -> str """
        return ""

    def title(self): # real signature unknown; restored from __doc__
        """ title(self) -> str """
        return ""

    def __init__(self, *__args): # real signature unknown; restored from __doc__ with multiple overloads
        pass


